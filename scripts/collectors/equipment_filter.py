...
# ---------------------------------------------------------------------------
# 화면에 그대로 보이는 값이라 한국어를 앞세우고 표준 약어만 괄호로 남긴다.
# 약어 단독("ALD")이나 영문 병기("열처리 / Annealing")는 쓰지 않는다
# (지시문 No.015 2·7번).
PROCESS_RULES = [
    ("습식 식각", ["습식 식각", "wet etch", "湿法刻蚀", "濕蝕刻", "spin etch"]),
    ("건식 식각", ["건식 식각", "dry etch", "干法刻蚀", "乾蝕刻"]),
    ("고심도 반응성이온식각(DRIE)", ["drie", "deep reactive ion"]),
    ("반응성이온식각(RIE)", ["rie", "반응성 이온"]),
    ("식각", ["식각", "etch", "etching", "刻蚀", "蝕刻"]),
    ("원자층증착(ALD)", ["ald", "원자층증착", "原子层沉积"]),
    ("화학기상증착(CVD)", ["cvd", "pecvd", "mocvd", "화학기상증착", "chemical vapor deposition"]),
    ("물리기상증착(PVD)", ["pvd", "물리기상증착"]),
    ("스퍼터링", ["스퍼터", "sputter", "濺鍍", "溅射"]),
    ("증착", ["증착", "deposition", "沉積", "沉积", "鍍膜", "镀膜", "蒸鍍"]),
    ("화학기계연마(CMP)", ["cmp", "화학기계연마", "화학기계적 평탄화"]),
    ("도금(ECD)", ["도금", "plating", "ecd", "電鍍", "电镀", "전해", "electroplat"]),
    ("세정", ["세정", "cleaning", "清洗", "clean"]),
    ("열처리", ["열처리", "어닐링", "anneal", "退火", "furnace", "퍼니스",
                            "성장로", "확산로", "rtp", "산질화막 성장"]),
    ("노광", ["노광", "리소그래피", "lithography", "曝光", "微影",
                            "stepper", "e-beam lithography", "전자빔 리소그래피"]),
    ("본딩", ["본딩", "bonding", "본더", "bonder", "鍵合", "键合", "접합"]),
    ("패키징", ["패키징", "packaging", "封裝", "封装"]),
    ("유리관통비아(TGV) 가공", ["tgv", "through glass via", "玻璃通孔", "유리 관통"]),
    ("레이저 비아 가공", ["레이저 가공", "레이저 드릴", "laser drill", "laser processing",
                           "激光加工", "雷射加工", "비아 가공"]),
...
FACILITY_TERMS = [
    ...
    # "모니터"는 넣지 않는다 — 한글은 부분 문자열로 맞추므로 "공정 모니터링",
    # "실시간 모니터링 시스템" 같은 정상 장비 공고까지 시설로 걸러 버린다.
    # 사무용 모니터 구매는 어차피 산업·장비 신호가 없어 제외된다.
    "사무기기", "복합기", "책상", "의자", "환경설비", "폐수",
    "배기", "스크러버 배관", "덕트 공사",
]

# ---------------------------------------------------------------------------
# 안전망 — "산업 무관"과 "산업 정보 부족"을 구분한다 (지시문 No.014 16~17번)
# ---------------------------------------------------------------------------
# 산업이 **명백히 다른 분야**임이 확인되는 신호. 장비처럼 보여도 우리 대상이
# 아니다. 아래 STRONG_EQUIPMENT_TERMS보다 먼저 본다 — "장비처럼 보인다"는
# 이유로 의료기기나 항공관제 시스템을 반도체 장비로 올리지 않기 위해서다.
#
# 한글은 부분 문자열로 맞추므로 짧은 단어를 넣으면 안 된다. 실제로
# "수의"를 넣었더니 정부 계약방식인 "소액수의계약"·"특정수의계약"에 걸려
# 멀쩡한 KANC 공고 3건이 "수의학 분야"로 분류됐다. 같은 이유로
# "제약"(→ 설계 제약), "사료"(→ 검사료), "조리"(→ 조리개)도 넣지 않는다.
# 반드시 그 분야에서만 쓰는 표현으로 적는다.
UNRELATED_INDUSTRY_TERMS = [
    "의료", "의료기기", "병원", "임상시험", "방사선 치료", "치료기", "수술",
    "의약품", "제약회사", "백신", "진단키트", "체외진단",
    "수의학", "수의사", "동물병원",
    "항공관제", "관제탑", "항행", "활주로",
    "경마", "마사회", "축산", "배합사료", "농기계", "식품", "급식",
    "자동차 정비", "정비고", "차량 정비",
    "상수도", "하수도", "정수장", "하수처리",
    "교육 훈련", "훈련 시뮬레이터", "체험관", "전시관",
]

# **독립 장비임이 매우 강한** 신호. 이게 있는데 산업 근거만 없는 경우는
# "아니다"가 아니라 "모른다"에 가깝다 — 상세 페이지를 못 읽었거나 제목이
# 짧아서 산업 단어가 안 보이는 것일 수 있다. 그래서 바로 제외하지 않고
# 검토 필요로 남긴다.
#
# 여기에는 **팹·라인 공정 장비를 특정하는 이름만** 넣는다. "장비", "시스템",
# "측정기", "레이저", "현미경" 같은 일반어는 넣지 않는다 — 넣으면 일반
# 연구장비가 전부 검토 필요로 쏟아져 검토 목록이 무의미해진다
# (지시문 No.014 18번). 같은 이유로 KRISS 중적외선 레이저 시스템은 여기에
# 걸리지 않고 제외를 유지한다(19번).
#
# 이미 PROCESS_INDUSTRY_TERMS에 있어 산업 신호로도 잡히는 이름(prober, AOI,
# CMP, MOCVD, furnace, RTP, wet bench 등)은 여기 없어도 장비로 판정된다.
# 아래는 그 목록에 없어서 실제로 자동 제외되던 것들이다.
STRONG_EQUIPMENT_TERMS = [
    "stepper", "스테퍼", "etcher", "식각기", "에처",
    "tester", "시험기", "테스터", "prober", "프로버",
    "plating system", "도금 장비", "도금 설비", "전해 도금 장비", "ecd",
    "inspection system", "검사 장비", "검사 설비", "검사기",
    "metrology system", "계측 장비", "계측기", "측장기",
    "plasma processing system", "플라즈마 처리 장비", "플라즈마 처리 시스템",
    "wafer bonder", "본더", "bonder", "다이 본더",
    "cleaning system", "세정기", "웨트 스테이션",
    "polisher", "연마 장비", "연마기", "cmp 장비",
    "coater", "코터", "트랙 장비", "developer",
    "ion implanter", "이온주입기", "임플란터",
    "epi reactor", "에피 장비", "확산로", "열처리로",
]
...
def classify(item):
    ...
    # 산업이 명백히 다른 분야로 확인되면 장비 신호가 아무리 강해도 제외한다.
    # "장비처럼 보인다"는 이유로 의료기기·항공관제를 올리지 않기 위해서다.
    unrelated = _first_match(text, UNRELATED_INDUSTRY_TERMS)
    if unrelated:
        return "제외", f"반도체/디스플레이/TGV와 다른 분야 (신호: {unrelated})"
    ...
    if not industry:
        ...
        # 여기서 "산업 근거가 없다"는 것은 "다른 분야임이 확인됐다"가 아니라
        # "아직 확인하지 못했다"일 수 있다. 상세 페이지를 못 읽었거나 제목이
        # 짧으면 산업 단어가 안 보인다. 그런데 사려는 물건이 팹 공정 장비를
        # 특정하는 이름이라면 그냥 버리기에는 아깝다 — 검토 필요로 남긴다.
        # (산업이 명백히 다른 경우는 위 unrelated 검사에서 이미 걸러졌다.)
        strong = _first_match(buy, STRONG_EQUIPMENT_TERMS)
        if strong:
            return "검토 필요", (f"팹 공정 장비 신호({strong})는 강하지만 "
                              f"반도체/디스플레이/TGV 산업 근거를 확인하지 못함 "
                              f"— 원문 확인 필요")

        if equipment:
            # 장비를 사는 것은 맞지만, 반도체/디스플레이/TGV 공정·산업과
            # 이어지는 근거가 공고 어디에도 없다. 이런 건은 대부분
            # 연구기관이 사는 일반 실험·계측 장비다(중적외선 레이저 분광
            # 시스템, 열분석기, 전자현미경 등).
            #
            # 기관이나 부서 이름이 반도체를 달고 있다는 이유로 포함하지
            # 않는다 — "반도체 관련 부서가 산 연구장비"와 "반도체용 장비"는
            # 다르기 때문이다(지시문 No.013 2번, KRISS 중적외선 레이저
            # 시스템 판정. 근거는 docs/REVIEW-KRISS-63846.md).
            #
            # 산업 근거는 INDUSTRY_TERMS(반도체·디스플레이·TGV·패키징)와
            # PROCESS_INDUSTRY_TERMS(식각·증착·노광·CMP·AOI·프로버 등)로
            # 넓게 잡아 두었다. 실제 반도체 장비 공고는 제목에 이 중 하나가
            # 거의 반드시 들어간다 — 그래서 여기까지 오는 건은 우리 대상이
            # 아닐 가능성이 높다.
            return "제외", (f"장비({equipment})이지만 반도체/디스플레이/TGV "
                            f"공정·산업 근거 없음 — 일반 연구·계측 장비")
