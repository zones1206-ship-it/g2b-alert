...
COMPANY_TERMS = {
    ...
    "长鑫存储": "CXMT(창신메모리)",
    "维信诺": "웨이신눠(Visionox)",
    # 회사명은 COMPANY_TERMS(가장 먼저 치환)에 통째로 둬야 한다.
    # "微电子"를 일반 용어로 넣은 뒤 "芯联微电子"가 "芯联"+마이크로전자로
    # 쪼개져 "XinLian 마이크로전자"가 된 적이 있다.
    "芯联微电子": "신롄 마이크로전자(Xinlian Microelectronics)",
    "芯联": "신롄(Xinlian)",
    # 공식 영문명은 회사 자체 도메인(eswin.com)에서 확인했다 —
    # "北京奕斯伟科技集团有限公司-官网". 한국어는 뜻을 지어낸 것이 아니라
    # 중국어 발음 음역이며, 이 사전의 다른 회사들과 같은 방식이다
    # (京东方 → 징둥팡, 华星光电 → 화싱광전, 维信诺 → 웨이신눠).
    "奕斯伟": "이쓰웨이(ESWIN)",
    # 공식 영문명을 확인하지 못해 음역만 쓴다.
    "奕成": "이청",
    "天马微电子": "Tianma(톈마)",
    "天马": "Tianma(톈마)",
    "国显科技": "궈셴과기",
...
TECH_TERMS = {
    ...
    "显示": "디스플레이",
    "光电子": "광전자",   # "光电"(같은 사전)보다 길어 먼저 치환된다
    "光电": "광전",
    "印刷": "인쇄",
    "高清": "고화질",
    "显示面板": "디스플레이 패널",
    ...
    # 위 6자 표기가 먼저 치환되므로 아래 짧은 표기와 충돌하지 않는다.
    # 2026-09-04 MOFCOM 신규 공고에서 로마자로 떨어졌던 디스플레이 용어들이다
    # (RouXing / YouYuan JuZhen / YouJi FaGuang).
    "有源矩阵": "액티브 매트릭스",
    "有机发光": "유기발광",
    "柔性": "플렉시블",
...
GENERIC_TERMS = {
    ...
    "前段": "전공정", "后段": "후공정",
    "科技": "과학기술",
    # 회사명 접미사로 흔히 붙는다("…电子有限公司" = "…전자 유한공사").
    # 긴 조합어를 함께 둬야 "微电子"가 "微전자"로 깨지지 않는다
    # (같은 사전 안에서는 긴 표기가 먼저 치환된다).
    "电子材料": "전자재료",
    "微电子": "마이크로전자",
    "电子": "전자",
    "研发": "연구개발",
