    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
        try:
            ...
            if not should_retry(exc):
                print(f"[KANC] 재시도하지 않는 오류로 즉시 중단: {exc}")
                break
            if attempt < MAX_RETRY_ATTEMPTS:
                delay = retry_delay(exc, RETRY_DELAY_SECONDS)
                print(f"[KANC] 요청 실패({exc}), {delay:.0f}초 후 재시도 {attempt}/{MAX_RETRY_ATTEMPTS - 1}")
                time.sleep(delay)
...
def extract_due_date(text: str, registered_date: str = None):
    ...
    #   "제출기간: 2026.08.28. 14:00 ~ 2026.09.07. 14:00"
    # "제출기간"은 아래 "...기간" 패턴이 날짜 사이의 시각(14:00)을 넘지 못해
    # 놓치던 표기다. 어느 표기든 마지막 날짜가 마감이다.
    submit = re.search(r"제출\s*(?:시한|기한|기간)\s*[:：]\s*([^\n]{0,70})", text)
    if submit:
