...
페이지 요청 사이에도 0.7초씩 쉬어 서버 부하를 줄인다. 재시도를 모두
소진해도 실패하면 어느 페이지에서 어떤 HTTP 상태로 실패했는지 로그로
남기고 기존 data/announcements.json은 건드리지 않은 채 종료한다.
페이지당 요청 건수(NUM_OF_ROWS)는 응답 크기가 커지면 502 가능성이
높아질 수 있어 500건으로 유지한다. 999로 늘리는 건 재시도/대기 로직이
안정적으로 동작하는 것을 확인한 뒤 별도로 검토한다.
"""
...
def fetch_page(service_key: str, begin_dt: str, end_dt: str, page_no: int, num_of_rows: int = NUM_OF_ROWS, total_pages=None):
    ...
    로그 예시:
      ...
      [13/32] 재시도 성공
    """
    prefix = f"[{page_no}/{total_pages}]" if total_pages else f"[{page_no}]"
