...
늘려가며(3초→10초→30초→60초→90초) 최대 5회까지 재시도하고, 페이지
사이에도 0.5초씩 쉬어 서버 부하를 줄인다. 재시도를 모두 소진해도
실패하면 어느 페이지에서 실패했는지 로그로 남기고 기존
data/announcements.json은 건드리지 않은 채 종료한다.
"""
...
LOOKBACK_DAYS = 30
NUM_OF_ROWS = 999  # 왕복 횟수를 줄이기 위해 페이지당 최대한 많이 요청
MAX_PAGES = 100

# 502/503/504 등 일시적 오류에 대한 재시도 설정 (요청 간 대기시간도 여기 포함)
MAX_RETRIES = 5
RETRY_DELAYS = [3, 10, 30, 60, 90]  # 재시도 사이 대기 시간(초), 점진적으로 증가
...
def request_once(service_key: str, begin_dt: str, end_dt: str, page_no: int, num_of_rows: int):
    ...
    try:
        ...
        if exc.code in RETRYABLE_HTTP_CODES:
            raise RetryableAPIError(f"HTTP {exc.code}: {body}") from exc
...
def fetch_page(service_key: str, begin_dt: str, end_dt: str, page_no: int, num_of_rows: int = NUM_OF_ROWS):
    """일시적 오류(502/503/504, 네트워크 오류)는 지수적으로 대기시간을 늘려가며 재시도한다."""
    last_error = None
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            if attempt > 1:
                print(f"  [페이지 {page_no}] 재시도 {attempt - 1}/{MAX_RETRIES - 1}...")
            return request_once(service_key, begin_dt, end_dt, page_no, num_of_rows)
        except RetryableAPIError as exc:
            last_error = exc
            if attempt < MAX_RETRIES:
                delay = RETRY_DELAYS[min(attempt - 1, len(RETRY_DELAYS) - 1)]
                print(f"  [페이지 {page_no}] 일시적 오류 발생({exc}), {delay}초 후 재시도합니다.")
                time.sleep(delay)
            else:
                print(f"  [페이지 {page_no}] {MAX_RETRIES}회 재시도 모두 실패: {exc}")

    raise RuntimeError(f"페이지 {page_no} 요청이 {MAX_RETRIES}회 재시도 후에도 실패했습니다: {last_error}")
...
def collect_items(service_key: str):
    ...
    while page_no <= MAX_PAGES:
        if total_pages:
            print(f"[페이지 {page_no}/{total_pages}] 요청 중...")
        else:
        ...
        if page_no == 1:
            ...
            print(f"총 {total_pages}페이지를 요청할 예정입니다.")
        if not rows:
            break
        raw_count += len(rows)
        ...
        if page_no * NUM_OF_ROWS >= total_count:
            break
        page_no += 1
        time.sleep(0.5)  # 연속 요청으로 인한 서버 부하/502 방지
