...
> KOTRA는 **GitHub Actions 러너 IP에 따른 선택적·부분적 접속 차단**이 있다.
> 로컬에서는 0.4초에 정상 응답한다. 러너에서 계층별로 진단한 결과 애플리케이션
> 레벨(헤더·요청 방식) 문제는 아니었다 — 막힌 러너에서는 DNS는 해석되지만
> TCP 443 연결이 timeout이고, `curl`이 `http=000`을 내며, 요청 조합 5가지와
> 비-AJAX 공개 HTML 페이지가 모두 동일하게 실패했다.
>
> 다만 **모든 러너에서 막히는 것은 아니다.** 야간 5회 반복 실행 결과 3회는
> 정상 수집됐다. 러너 IP에 따라 셋 중 하나다.
>
> - 통과 IP: 정상 수집된다.
> - 불안정 IP: `RemoteDisconnected`가 자주 나지만 재시도로 회복된다.
> - 차단 IP: TCP 443 단계에서 timeout이며 재시도로도 회복되지 않는다.
>
> 차단 우회는 하지 않는다. 차단 IP를 만난 실행에서는 KOTRA만 실패로 기록하고
> 이전에 수집된 데이터를 유지한 채 아래 Health Check로 사실을 노출한다.
> 자세한 내용은 [docs/OPERATIONS.md](docs/OPERATIONS.md) 5절.
