...
반도체·디스플레이·도금/TGV 장비를 실제로 구매·제작·설치하는 전문
...
KDIA(한국디스플레이산업협회), KOTRA는 중국 디스플레이/반도체 장비
프로젝트 정보원으로 검토했으나, 조사 결과 로그인/우회 없이 자동
수집 가능한 공개 입찰·비딩 게시판을 찾지 못해 보류 중이다
(KDIA는 월간 산업동향 리포트만 존재, KOTRA는 국가정보/시장뉴스
API만 존재). 실제 게시판이 확인되면 `collectors/`에 추가한다.
...
- KANC는 이미 반도체/나노 장비 전문 게시판이라 카테고리 분류 시 넓은
  필터를 쓰지 않고, 디스플레이/도금 신호가 명확할 때만 그쪽으로 분류하고
