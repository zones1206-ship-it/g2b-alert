...
- `.github/workflows/fetch-announcements.yml`이 매일 07:00(KST)에 실행되어
  ...
  카테고리(반도체 장비, 디스플레이 장비, 도금 장비)에 매칭되는 공고만
  `data/announcements.json`에 저장한다.
- 사용 API: `BidPublicInfoService` / `getBidPblancListInfoServc`
  (`https://apis.data.go.kr/1230000/ad/BidPublicInfoService/getBidPblancListInfoServc`)
- 나라장터 API의 공고명 검색 파라미터(`bidNtceNm`)는 실제로는 서버에서
  제목을 필터링해주지 않는 것으로 확인되어 사용하지 않는다. 대신 조회 기간
  내 전체 공고를 한 번의 페이지네이션으로 받아온 뒤,
  `scripts/fetch_announcements.py`의 `CATEGORY_MATCH_TERMS` 사전으로
  로컬에서 제목을 분류한다.
- 카테고리별 내부 매칭 사전(`CATEGORY_MATCH_TERMS`)에는 카테고리명 자체가
  아니라 세부 검색어(예: 반도체 장비 → 웨이퍼, wafer, EFEM, FOUP, 클린룸 등)가
  등록되어 있다. 공고 제목에 이 세부 검색어 중 하나라도 포함되면 매칭되며,
  한 공고가 여러 카테고리에 매칭되면 `keywords` 배열에 함께 저장되어 결과
  화면에서 여러 그룹에 동시에 표시된다.
- 실행 지연·실패에 대비해 최근 30일치를 조회한다.
- 저장소 Actions 탭에서 `Run workflow`로 즉시 수동 실행도 가능하다 (workflow_dispatch).
  API 연동이 정상인지 처음엔 수동 실행으로 확인하는 것을 권장한다.
- 데이터가 갱신되면 워크플로우가 직접 커밋/푸시하므로 별도 배포 단계 없이
  GitHub Pages가 자동으로 최신 화면을 서빙한다.
- 인증키가 잘못되었거나 활용신청이 아직 승인되지 않은 경우 API가 JSON 대신
  XML 에러를 반환하는데, 스크립트가 이를 감지해 에러 로그를 남기고
  기존 `data/announcements.json`은 건드리지 않은 채 종료한다.
  Actions 탭의 실행 로그에서 에러 메시지를 확인할 수 있다.
