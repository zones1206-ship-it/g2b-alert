...
## 실행 검증 뒤 추가로 찾은 것 (지시 37번에 따른 3차 점검)

### 1. 공고번호가 같은데 별개인 공고가 있다

`project_key`(출처+공고번호)를 "같으면 버린다"는 근거로 쓴 것이 틀렸다.
현재 데이터에 같은 공고번호를 가진 별개 항목이 5쌍 있다.

| 공고번호 | 함께 올라온 항목 |
|---|---|
| EBNEW 0664-2640SUMECF94 | 2차 입찰공고(1) / 2차 입찰공고(2) / 2차 재입찰 정정·변경 공고(1) |
| EBNEW 0692-2640XM075270/01 | 원공고 / 재입찰 정정·변경 공고 |
| EBNEW 0692-2640XM075270/04 | 원공고 / 재입찰 정정·변경 공고 |
| EBNEW 4197-2640CSOTGZT8/124 | 원공고 / 재입찰 정정·변경 공고 |
| MOFCOM 4197-264BOECDCZ02/03 | 국제 입찰공고(1) 마감 09-16 / (2) 마감 09-24 |

원공고가 목록 범위 밖으로 밀린 날, 같은 번호의 정정공고가 수집되면
원공고를 버렸다. **진행 중 공고가 사라지는 경로**다.

고친 방식 — 공고번호는 **지키는 근거로만** 쓴다.

- 번호가 이번 수집분에 없다 → 확실히 별개 공고다. 유지한다.
- 그 외 → 제목·기관·게시일·마감일·공고종류가 모두 같을 때(내부 id만 바뀐
  재색인)에만 버린다.

실제 데이터로 재현: 원공고 `ebnew709582888`(마감 09-11)이 밀린 상황에서
고치기 전에는 버려졌고, 고친 뒤에는 유지된다.

### 2. KANC "제출기간" 표기

`제출기간: 2026.08.28. 14:00 ~ 2026.09.07. 14:00` — 두 날짜 사이에 시각이
있어 기존 `...기간` 패턴이 넘지 못했다. 제출 라벨에 `기간`을 더해 해결.
공고 제2026-31호 마감 2026-09-07 복구.

### 3. 마감일이 없는 채로 남은 9건 — 원문에 실제로 없다

원문을 직접 열어 확인했다. 추측으로 채우지 않는다.

| 출처 | 건수 | 원문 근거 |
|---|---|---|
| EBNEW 정정·변경 공고 | 4 | `截止时间：` 필드가 비어 있음 |
| EBNEW 낙찰·수주결과 | 4 | 같음. 결과 공고라 마감 개념이 없음 |
| KOTRA 수출상담회 | 1 | "연중 신청 가능" 상시 사업. 사업기간(2026-01-01~12-31)은 있으나 신청 마감일이 아니다 |

KOTRA의 사업기간 종료일을 마감일로 쓰는 것은 "행사 종료일을 마감일로
추측"에 해당하므로 하지 않았다.
